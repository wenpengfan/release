...
for i in $service; do
echo ========================关闭进程=============================
ssh -p 12525 trade@$i "ps -ef|grep "tomcat_$1"|grep -v grep"
if [ $? != 0 ]
        then
                echo '进程为关闭状态!'
        else
...
echo =======================回滚开始=====================================
path="cd /data/trade/realweb/tomcat_$1/webapps/$1/"
...
echo ===========================程序启动=============================
ssh -p 12525 trade@$i "export BASH_ENV=/etc/profile && cd /data/trade/realweb/tomcat_$1 && bash quikstat.sh && ps -ef|grep $1"
if [ $? != 0 ]
        then
                echo "-1,启动程序失败,日志完成"
		exit
